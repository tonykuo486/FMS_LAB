...
（**完整鎖定版本見 [`docs/SDD.md`](docs/SDD.md) §11——2026-08-25 實測核實**）
**必讀三份文件**（各司其職，別混著讀）：

| 文件 | 全名 | 讀它做什麼 |
|---|---|---|
| [`docs/SRS.md`](docs/SRS.md) | Software Requirements Specification<br>軟體需求規格書 | **要做什麼**——第 2 章每條編號都是驗收項 |
| [`docs/SDD.md`](docs/SDD.md) | Software Design Document<br>軟體設計文件 | **怎麼做**——架構、選型、部署、版本鎖定 |
...
**1. 精讀 SRS 第 2 章（需求），再讀 SDD（設計）。** SDD 這五節是硬規格，不是建議：

| SDD 節 | 內容 | 為何不能略過 |
|---|---|---|
| §3 | 專案結構（monorepo） | 目錄長什麼樣是驗收項，不是品味問題 |
| §4 | 權限矩陣（RBAC） | 前端擋 UX、**後端擋安全**——兩邊都要 |
| §7 | SQL 資產化規範 | 每條 SQL 要有 `source:` 反查需求編號 |
...
```bash
bun --version        # 需 Bun 1.4.0（版本鎖定表見 [SDD §11](docs/SDD.md)）
...
建測試用資料庫（[SDD §6](docs/SDD.md)：測試禁連正式庫），**只做一次**：
...
> （`db` + `backend` + `frontend`）的 compose，規格見 [SDD §9](docs/SDD.md)——那份要**你自己寫**，
...
|---|---|
| 開專案 | `bun init` |
| 加依賴 | `bun add elysia@1.4.29 pg@8.23.0`（**一律寫死版本,見 [SDD §11](docs/SDD.md)**） |
...
照 [`docs/SDD.md`](docs/SDD.md) **§11 版本鎖定表**把 `package.json` 一次寫好
...
必須在 **SQL 層**就以別名輸出（見 [SDD §7](docs/SDD.md) 的 `dense_rank()` 範例），
API 從頭到尾不含真名。

**連線池與測試隔離（[SDD §5／§6](docs/SDD.md)）。** 後端一律用 `db/pool.ts` 的連線池單例，
**禁止**每個請求開新連線（會 `too many connections`）。測試**禁連正式庫**——
用 `fms_test` 資料庫，每個測試檔開自己的 schema，跑完 `DROP SCHEMA … CASCADE`。

**排程 race 是必測項（[SDD §5](docs/SDD.md)）。** 換成真 PG 之後，兩條連線同時搶同一工作站同一時段
...
複製過來的有 `README.md`、`.gitignore`、`infra/`（環境）與 `docs/`（SRS/SDD/GUIDE 三份）
