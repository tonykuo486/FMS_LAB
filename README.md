...
FMS_LAB/
├── README.md                 ← 本檔（入口導覽）
├── CLAUDE.md                 ← 教材維護視角（維護鐵律；學員看各版內的）
├── .gitignore
├── docs/
│   └── V2_V3_對照表.md       ← 兩版完整對照（講師備課/學員課後對讀）
├── FMS_V2/                   ← V2 教材（Python 路線）
│   ├── README.md             ← 門面：開工指引＋轉交自己 GitHub 的步驟
│   ├── CLAUDE.md             ← 工程底冊（給 AI 與你；鐵律＋協作節奏）
│   ├── CHANGELOG.md          ← ★你的作業時間軸（模板＋示範條目）
...
│       └── 99_決策紀錄_ADR.md ← ★你的設計決策（ADR-0 示範，從 ADR-1 寫起）
...
> 每版四件套＝家族慣例：**README 門面／CLAUDE 技術底冊／CHANGELOG 變化歷程／
> docs/99 ADR 決策**——與正式產品線同一套紀律，學員在課堂上練的就是業界的做法。
...
5. **邊做邊記兩個檔**（2026-08-25 補齊，各版都有模板與示範）：`CHANGELOG.md`
   （每完成一個里程碑寫一條：變化點＋驗證證據）＋ `docs/99_決策紀錄_ADR.md`（至少 2 條你自己的
   設計決策：背景→選項→決定→代價）。這兩個檔＋核實紀錄式 commit＝業界「可交接
   專案」的最小配備，老師抽查 git log 時一併看。
